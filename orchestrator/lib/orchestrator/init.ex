defmodule Orchestrator.Init do
  @moduledoc """
  Scaffolds a `.orchestrator/` workspace by copying packaged templates from `priv/`.
  """

  @templates_rel ["priv", "templates", "orchestrator"]

  @doc """
  Copies template files into `<workspace>/.orchestrator/`.

  Workspace root is `ORCHESTRATOR_WORKSPACE` when set, otherwise the current working directory.
  ...
  def run(opts \\ []) do
    ...
    src = templates_root()
    dest = Path.join(workspace, ".orchestrator")
  ...
  defp templates_root do
    Application.app_dir(:orchestrator, @templates_rel)
  end
