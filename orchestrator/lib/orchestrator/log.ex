defmodule Orchestrator.Log do
  ...
  Level is controlled by `ORCHESTRATOR_LOG_LEVEL` (default `INFO`):
  ...
  @doc "Configures Logger and application log threshold from `ORCHESTRATOR_LOG_LEVEL`."
  @spec configure!() :: :ok
  def configure! do
    level = configured_level()
    Application.put_env(:orchestrator, :log_level, level)
  ...
  def configured_level do
    System.get_env("ORCHESTRATOR_LOG_LEVEL", "INFO")
    |> String.trim()
    |> String.downcase()
    |> case do
      "trace" -> :trace
      ...
      other ->
        Logger.warning(
          "invalid ORCHESTRATOR_LOG_LEVEL=#{inspect(other)}, using info"
        )
  ...
  @spec run_metadata(Orchestrator.Workflow.RunContext.t()) :: keyword()
  def run_metadata(%Orchestrator.Workflow.RunContext{} = ctx) do
    metadata(run_id: ctx.run_id, workspace: ctx.workspace_root)
  end

  defp enrich_metadata(:trace, metadata),
    do: Keyword.put(metadata, :orchestrator_level, :trace)
