defmodule Definitively.InitTest do
  use ExUnit.Case, async: false

  import ExUnit.CaptureIO

  alias Definitively.{CLI, Init}
  ...
  test "run copies template files into .definitively", %{workspace: workspace} do
...
")
  ...
  test "run uses DEFINITIVELY_WORKSPACE from env", %{workspace: workspace} do
  ...
  test "dispatch init scaffolds workspace", %{workspace: workspace} do
    ...
    assert File.regular?(Path.join([workspace, ".definitively", "programs", "example.yml"]))
  end
